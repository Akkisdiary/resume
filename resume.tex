...
\resumeSubHeadingListStart
    ...
    \resumeItemListStart
        \resumeItem
        {Enhanced and maintained data extraction platform. Delivering e-commerce data to \textbf{Fortune 500 companies} that enable clients to make informed business decisions, optimize their operations, and gain competitive market insights.}
        \resumeItem
        {Migrated the legacy data extraction platform to \textbf{Amazon EKS}, leveraging \textbf{Kubernetes and Docker} to improve scalability and performance. Combined multiple systems into one platform that processes over \textbf{200 million} daily requests.}
        \resumeItem
        {Solved \textbf{AWS VPC IP} shortages by designing an \textbf{Auto Scaling Group (ASG)} for Kubernetes pods, enabling smooth deployments and better scalability.}
        \resumeItem
        {Optimized \textbf{CI/CD pipelines} for microservices using \textbf{GitHub Workflows}, ensuring smoother deployments and better system reliability.}
        \resumeItem
        {Enhanced platform monitoring by migrating existing observability and monitoring tools to \textbf{Coralogix}, improving visibility and reducing operational costs.}
        \resumeItem
        {Developed a solution to install telemetry agents in worker machines using \textbf{Ansible \& AWX}, providing customized configurations for various teams.}
        \resumeItem
        {Developed multiple microservices to support the new architecture, streamlining the data extraction process and enhancing platform functionality with new API endpoints.}
        ...
        \resumeItem
        {Developed \textbf{50+ web scraping bots} using \textbf{Python and LXML HTML Parser} to gather location-specific data efficiently.}
        \resumeItem
        {Built techniques to bypass advanced bot detection techniques like \textbf{Cloudflare}, boosting success rates and saving resources.}
        \resumeItem
        {Designed a monitoring tool using \textbf{React JS and TypeScript}, allowing faster issue detection and resolution for web scraping bots.}
    \resumeItemListEnd
